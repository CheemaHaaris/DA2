...
# seq etc in graphs
# cities data - try to find online for fpl team
...
ggplot(data = final_df, aes(x= yc, y= tgc)) +
  geom_smooth(method="loess", formula = y ~ x) +
    geom_point(col= "red") +
...
# Error 
ggplot(data = final_df, aes(x= rc, y= tgc)) +
  geom_smooth(method="loess", formula = y ~ x) +
  #scale_x_continuous(expand = c(0.01,0.01), limits = c(1,5), breaks = seq(1,5,1))+
  #scale_y_continuous(expand = c(0.01,0.01), limits = c(0,1), breaks = seq(0,1,0.1)) +
  labs(x = "Number of Red Cards",y = "Total Number of Goals") 
  
 
ggplot(data = final_df, aes(x= shots, y= tgc)) +
  geom_smooth(method="loess", formula = y ~ x) +
    geom_point( col = "red") +
      annotate("text", x = 15, y = 3.5, label = "More or less linear positive association", size=4) +
  #scale_x_continuous(expand = c(0.01,0.01), limits = c(1,5), breaks = seq(1,5,1))+
  #scale_y_continuous(expand = c(0.01,0.01), limits = c(0,1), breaks = seq(0,1,0.1)) +
            labs(x = "Total Number of Shots",y = "Total Number of Goals ") 
  

ggplot(data = final_df, aes(x = shots_t, y = tgc)) +
  geom_smooth(method="loess", formula = y ~ x) +
    geom_point(col = "red") +
...
# Regressing x on confounding variables

xreg <- feols( rain ~ cnr + yc + rc + shots + shots_t, data = final_df, vcov = 'hetero')
xreg

# Main Regressions

reg1 <- feols( tgc ~ rain + cnr + yc + shots + shots_t , data = final_df, vcov = 'hetero')

reg2 <- feols( tgc ~ rain + cnr + yc + rc + shots + shots_t  , data = final_df , vcov = 'hetero' )

reg3 <- feols( tgc ~ rain + cnr + lspline(yc,c(4,6)) + shots + lspline(shots_t,17)  , data = final_df , vcov = 'hetero' )

reg4 <- feols( tgc ~ rain + cnr + lspline(yc,c(4,6)) + rc + shots + lspline(shots_t,17)  , data = final_df , vcov = 'hetero' )
